...
def generate_launch_description():
    ...
    # Conditionally include AMCL in lifecycle nodes
    lifecycle_nodes = ['map_server']
    # Add AMCL to lifecycle nodes if enabled
    ...
    declare_use_amcl_cmd = DeclareLaunchArgument(
        'use_amcl',
        default_value='True',
        description='Whether to enable AMCL or not',
    )
    ...
    load_nodes = GroupAction(
        ...
        actions=[
            ...
            Node(
                ...
                output='screen',
                arguments=['--ros-args', '--log-level', log_level],
                parameters=[{'autostart': autostart}, {'node_names': lifecycle_nodes_with_amcl}],
            ),
            Node(
                condition=IfCondition(NotEqualsSubstitution(use_amcl, 'true')),
                package='nav2_lifecycle_manager',
                executable='lifecycle_manager',
                name='lifecycle_manager_localization',
                output='screen',
            ...
            LoadComposableNodes(
                ...
                composable_node_descriptions=[
                    ...
                    ComposableNode(
                        ...
                        name='lifecycle_manager_localization',
                        parameters=[
                            {'autostart': autostart, 'node_names': lifecycle_nodes_with_amcl}
                        ],
                    ),
                    ComposableNode(
                        condition=IfCondition(NotEqualsSubstitution(use_amcl, 'true')),
